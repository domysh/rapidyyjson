 ...
 *
 * Pretty JSON writer, mirroring `rapidjson/prettywriter.h`.
 */
    ...
    : public Writer<OutputStream, SourceEncoding, TargetEncoding, StackAllocator, writeFlags>
{
  public:
    ...
    */
    ...
    {
        RAPIDYYJSON_ASSERT(indentChar == ' ' || indentChar == '\t' || indentChar == '\n' ||
                           indentChar == '\r');
    ...
    */
    ...
    bool EndObject(SizeType memberCount = 0)
    {
        ...
        RAPIDYYJSON_ASSERT(Base::level_stack_.GetSize() >= sizeof(typename Base::Level));
        // currently inside an Array, not Object
        RAPIDYYJSON_ASSERT(!Base::level_stack_.template Top<typename Base::Level>()->inArray);
        // Object has a Key without a Value
        RAPIDYYJSON_ASSERT(0 ==
                           Base::level_stack_.template Top<typename Base::Level>()->valueCount % 2);
  ...
  protected:
    void PrettyPrefix(Type type)
    {
        ...
        {
            RAPIDYYJSON_ASSERT(!Base::hasRoot_); // Should only has one and only one root.
            Base::hasRoot_ = true;
        }
    }
