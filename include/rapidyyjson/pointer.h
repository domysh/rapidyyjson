 ...
 *
 * JSON Pointer (RFC 6901), mirroring `rapidjson/pointer.h`.
 */
...
static const SizeType kPointerInvalidIndex = ~SizeType(0); //!< Represents an invalid index in
...
/*!
    ...
    \tparam ValueType The value type of the DOM tree. E.g. GenericDocument<UTF8<> >
...
class GenericPointer
{
  public:
    ...
    struct Token
    {
        const Ch* name;  //!< Name of the token. It has null character at the end but it can
    ...
    */
    ...
    GenericPointer& Swap(GenericPointer& other) RAPIDYYJSON_NOEXCEPT
    {
    ...
    friend inline void swap(GenericPointer& a, GenericPointer& b) RAPIDYYJSON_NOEXCEPT
    {
        a.Swap(b);
    }
    ...
    GenericPointer Append(const ValueType& token, Allocator* allocator = 0) const
    {
        ...
        {
            RAPIDYYJSON_ASSERT(token.IsUint64());
            RAPIDYYJSON_ASSERT(token.GetUint64() <= SizeType(~0));
    ...
    bool operator<(const GenericPointer& rhs) const
    {
        ...
        for (size_t i = 0; i < tokenCount_; i++)
        {
            ...
            if (int cmp = std::memcmp(tokens_[i].name,
                                      rhs.tokens_[i].name,
            ...
            else
            {
                ...
                else
                {
                    typename ValueType::MemberIterator m = v->FindMember(
                        GenericValue<EncodingType, typename ValueType::AllocatorType>(
        ...
        for (const Token* t = tokens_; t != tokens_ + tokenCount_; ++t)
        {
            ...
            case kObjectType: {
                typename ValueType::MemberIterator m =
                    v->FindMember(GenericValue<EncodingType, typename ValueType::AllocatorType>(
                        GenericStringRef<Ch>(t->name, t->length)));
                if (m == v->MemberEnd())
                    break;
                v = &m->value;
                              ...
                              typename ValueType::AllocatorType& allocator) const
    {
        bool alreadyExist;
        ValueType& v = Create(root, allocator, &alreadyExist);
        return alreadyExist ? v : (v = ValueType(defaultValue, allocator).Move());
    }
...
#if RAPIDYYJSON_HAS_STDSTRING
    //! Set a std::basic_string in a document.
    template <typename stackAllocator>
    ValueType& Set(
        GenericDocument<EncodingType, typename ValueType::AllocatorType, stackAllocator>& document,
    ...
    Set(GenericDocument<EncodingType, typename ValueType::AllocatorType, stackAllocator>& document,
        T value) const
    {
        return Create(document) = ValueType(value).Move();
    }
  ...
  private:
    ...
    */
    ...
    //! Check whether a character should be percent-encoded, according to RFC 3986 2.3
    ...
    {
        RAPIDYYJSON_ASSERT(source != NULL);
        RAPIDYYJSON_ASSERT(nameBuffer_ == 0);
        RAPIDYYJSON_ASSERT(tokens_ == 0);
        ...
        Token* token = tokens_ = static_cast<Token*>(
            allocator_->Malloc(tokenCount_ * sizeof(Token) + (length + 1) * sizeof(Ch)));
        ...
        RAPIDYYJSON_ASSERT(name <= nameBuffer_ + length + 1); // Should not overflow buffer
        parseErrorCode_ = kPointerParseErrorNone;
        return;
...
template <typename T, typename CharType, size_t N>
bool
EraseValueByPointer(T& root, const CharType (&source)[N])
{
