 ...
 *
 * JSON writer (SAX handler), mirroring `rapidjson/writer.h`.
 */
...
enum WriteFlag
{
    ...
    kWriteDefaultFlags = RAPIDYYJSON_WRITE_DEFAULT_FLAGS //!< Default write flags.
};
...
class Writer
{
  public:
    ...
    */
    ...
    /*!
        ...
        The default setting does not truncate any decimal places. You can restore to this setting
        by calling
        \code
    ...
    */
    ...
    bool EndObject(SizeType memberCount = 0)
    {
        ...
        RAPIDYYJSON_ASSERT(level_stack_.GetSize() >= sizeof(Level));
        // currently inside an Array, not Object
        RAPIDYYJSON_ASSERT(!level_stack_.template Top<Level>()->inArray);
        // Object has a Key without a Value
        RAPIDYYJSON_ASSERT(0 == level_stack_.template Top<Level>()->valueCount % 2);
    ...
    /*!
        ...
        \param json A well-formed JSON value. It should not contain null character within
                    [0, length - 1] range.
  ...
  protected:
    ...
    {
        static const typename OutputStream::Ch hexDigits[16] = {'0',
                                                                '1',
                                                                '2',
                                                                '3',
                                                                '4',
...
#undef Z16
        ...
        while (RAPIDYYJSON_LIKELY(is.Tell() < length))
        {
            const Ch c = is.Peek();
            if (!TargetEncoding::supportUnicode && static_cast<unsigned>(c) >= 0x80)
            {
                // Unicode escaping
                unsigned codepoint;
                if (RAPIDYYJSON_UNLIKELY(!SourceEncoding::Decode(is, &codepoint)))
                    return false;
                PutUnsafe(*os_, '\\');
                PutUnsafe(*os_, 'u');
                ...
                {
                    RAPIDYYJSON_ASSERT(codepoint >= 0x010000 && codepoint <= 0x10FFFF);
                    // Surrogate pair
                    ...
                    PutUnsafe(*os_, hexDigits[(trail) & 15]);
                     ...
                     RAPIDYYJSON_UNLIKELY(escape[static_cast<unsigned char>(c)]))
            {
                is.Take();
                PutUnsafe(*os_, '\\');
                PutUnsafe(*os_,
    ...
    bool WriteRawValue(const Ch* json, size_t length)
    {
        ...
        while (RAPIDYYJSON_LIKELY(is.Tell() < length))
        {
            RAPIDYYJSON_ASSERT(is.Peek() != '\0');
            if (RAPIDYYJSON_UNLIKELY(
                    !(writeFlags & kWriteValidateEncodingFlag
                          ? Transcoder<SourceEncoding, TargetEncoding>::Validate(is, *os_)
    ...
    {
        (void)type;
        if (RAPIDYYJSON_LIKELY(level_stack_.GetSize() != 0))
        { // this value is not at root
            ...
            if (!level->inArray && level->valueCount % 2 == 0)
                RAPIDYYJSON_ASSERT(type == kStringType); // if it's in object, then even number
                                                         // should be a name
            level->valueCount++;
        }
        else
        {
            RAPIDYYJSON_ASSERT(!hasRoot_); // Should only has one and only one root.
            hasRoot_ = true;
        }
    }
